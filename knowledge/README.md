...
Agent rules: `AGENTS_RULES.md`. Read order is always this index first, then selected notes only,
then graph/backlink fallback. No full vault load.
...
<!-- INDEX START -->
- [Workflow tool delivers `args` as a JSON-encoded string, not an object](inbox/workflow-tool-args-string-bug.md) — confirmed 2x tool-level bug in idea-stresstest.js hard guard; unproven workaround pending 3rd confirmation · workflow-tool, bug, idea-stresstest, lesson, instinct-candidate · 2026-06-07
<!-- INDEX END -->
...
All notes follow `_templates/note.md` — title · summary · type/status/owner · privacy/source
metadata · graph fields · content · [[related]].
...
- Nova-V may read selected notes for evidence context, but does not write to `knowledge/`
