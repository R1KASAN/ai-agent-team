...
- มี **Level 1 Runtime** สำหรับดูสถานะงาน: `runtime_tracking` ใน `workflow_plan.md` และ
  `logs/runtime_status.md`
- งาน `dynamic_workflow` หรือ parallel/fanout ใช้ได้เฉพาะเมื่อ Rika-Chan approve ตาม gate

สรุป: เป็น multi-agent orchestrator แล้ว แต่ไม่ใช่ระบบ parallel อัตโนมัติเต็มรูปแบบ ระบบนี้ตั้งใจให้ปลอดภัยกว่าโดยมี artifact gate, runtime status, และ approval gate คั่นทุกช่วงสำคัญ
...
### Telegram Gateway v1 — inbox + approval queue

Telegram Gateway v1 เป็นช่องทางสั่งงานแบบประหยัด token ไม่ใช่ agent ใหม่:

- `/idea <text>` → map เป็น `/idea-gate` พร้อม `job_hint: idea-debate`, default `budget_cap: small`
- `/ask yuki <question>` → map เป็น `/idea-gate` consult, `target_agent: yuki`, default
  `budget_cap: tiny`
- `/status`, `/approve`, `/reject`, `/budget` → เปลี่ยนหรืออ่าน queue state เท่านั้น ไม่เรียก LLM

queue อยู่ที่ `runtime/queue/<run_id>.md` แบบหนึ่งไฟล์ต่อ request. Worker manual:
...
ถ้าจะต่อกับ Telegram Bot API จริงแบบ poll ครั้งเดียว:

```bash
TELEGRAM_BOT_TOKEN=... TELEGRAM_ALLOWED_USER_ID=... bash scripts/telegram_gateway_poll_once.sh
```

ถ้า worker ไม่เปิด ระบบไม่รันเอง แค่คงสถานะ pending. ถ้า budget ไม่พอ ระบบจะแจ้งว่า:
`run นี้หมด budget แล้ว; ใช้ /budget <run_id> medium แล้ว /approve <run_id> ถ้าต้องการให้ต่อ`

ข้อห้าม: Telegram v1 ไม่ใช่ long-chat memory, ไม่บันทึก chat history ทั้งก้อน, ไม่ทำ 24/7
daemon, ไม่เปิด parallel/fanout, และไม่รองรับ `large`/`dynamic` ผ่าน Telegram.
...
ถ้า `/idea-gate` ไปไกลกว่า pure classification, Minori ต้องเพิ่ม `runtime_tracking` และบันทึกสถานะสั้นๆ ลง `logs/runtime_status.md`
...
### Level 1 Runtime — ดูสถานะได้ แต่ไม่เพิ่ม agent

ระบบมี Runtime Layer ระดับ 1 แบบประหยัด token:

- `runtime_tracking` อยู่ใน `workflow_plan.md`
- `logs/runtime_status.md` เก็บแถวสั้นๆ ว่า run ไหนอยู่สถานะอะไร
- `latest_artifact` เก็บ path ของ artifact ล่าสุด ไม่ copy เนื้อหาทั้งไฟล์
- `next_step` คือ queue/status marker ไม่ใช่ scheduler จริง

สถานะที่ใช้: `planned`, `running`, `awaiting_approval`, `blocked`, `complete`, `aborted`

ข้อห้าม: Level 1 Runtime ห้าม spawn agent เพิ่ม ห้ามเปิด parallel/fanout ห้ามเปลี่ยน routing และห้ามบันทึก full chat/source/vault ลง log

สรุปสั้นๆ: ใช้เพื่อมองเห็น workflow ไม่ใช่ใช้เพื่อรัน workflow แทน Minori

### Telegram Gateway v1 — คุยผ่าน inbox แต่ยังคุม token

ถ้าคุยผ่าน Telegram ให้มองว่า Telegram เป็น **inbox + approval console**:

- สร้าง queue item ใน `runtime/queue/`
- ใช้ `budget_cap` เพื่อกันงานเล็กไหลเป็นงานแพง
- ใช้ `scripts/telegram_gateway_poll_once.sh` เป็น optional poller; ต้องส่ง token ผ่าน env เท่านั้น
- ใช้ `scripts/telegram_worker_run_once.sh` เพื่อ process งานที่ approve แล้วทีละ 1 item
- ผลลัพธ์คือ dispatch artifact สำหรับเปิดต่อใน Claude/Codex session ไม่ใช่การยิง LLM จาก Telegram

`/status`, `/approve`, `/reject`, `/budget` ต้องไม่เรียก LLM เสมอ.
