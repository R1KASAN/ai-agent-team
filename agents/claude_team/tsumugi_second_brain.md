...
> Read `knowledge/README.md` first, then targeted note — never load the full vault.
...
- Return a **scoped** `resurface_pack` for the **current step's topic only** — never load the full
...
When reading selected notes, use frontmatter to scope recall:
...
Sensitive/restricted notes require owner confirmation before surfacing. Schema filters are a recall
aid, not a reason to read the full vault.
...
- Proactive mode is scoped to the current step only — use only the README index, selected matching notes, and selected ledger rows.
